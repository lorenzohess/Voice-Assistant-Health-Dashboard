...
** Sample Data
*** Adding Sample Data
Populate the database with sample entries for testing/demo:
#+BEGIN_SRC bash
source venv/bin/activate
python scripts/add_sample_data.py
#+END_SRC
This creates 30 days of randomized data for all metrics plus a "Vegetable Servings" custom metric.
*** Removing Sample Data
Clear all health data entries (keeps custom metric definitions):
#+BEGIN_SRC bash
source venv/bin/activate
...
# Skip confirmation
python scripts/clear_data.py -y

# Also delete meal presets
python scripts/clear_data.py -y --include-presets
#+END_SRC
